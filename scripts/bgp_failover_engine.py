#!/usr/bin/env python3
"""
BGP Failover Engine VERSIÓN CON UMBRALES DINÁMICOS (Z-SCORE)
- Lógica de switch_margin CORRECTA
- Persistencia de cycle_number y current_provider
- TimescaleDB integrado
- ✅ NUEVO: Detección de anomalías basada en Z-score (umbrales dinámicos)
- ✅ NUEVO: Historial expandido a 10 ciclos para rolling statistics
...
# ✅ NUEVO: Umbrales de Z-score para detección de anomalías
# Basados en análisis estadístico (95% de los datos están dentro de ±2σ)
Z_SCORE_THRESHOLDS = {
...
# ✅ NUEVO: Tamaño del historial para rolling statistics
# 10 ciclos × 30s = 5 minutos de ventana
    ...
    def send_unified_metrics(self, cycle_data: Dict[str, Any], all_providers_metrics: Dict[str, Dict]):
        ...
        try:
            ...
            for provider_name, metrics in all_providers_metrics.items():
                ...
                # ✅ NUEVO: Métricas de Z-score para observabilidad
...
class BGPFailoverEngine:
    def __init__(self):
        ...
        # ✅ CORRECCIÓN: Expandir historial a ROLLING_HISTORY_SIZE (10 ciclos)
        ...
        # ✅ NUEVO: Tracking de anomalías Z-score
        self.z_score_anomaly_detected = {provider: False for provider in PROVIDERS}
    ...
    # ✅ NUEVO: Métodos para detección de anomalías Z-score
    def calculate_rolling_stats(self, provider: str) -> Dict[str, float]:
        ...
        Retorna dict con mean, std, p95 de peer_latency_ms.
        """
    ...
    def calculate_z_score(self, current_value: float, mean: float, std: float) -> float:
        ...
        """
        if std < 0.001:  # Evitar división por cero
            return 0.0
        return (current_value - mean) / std

    def detect_anomaly_z_score(self, provider: str, metrics: LatencyMetrics) -> Dict[str, Any]:
        """
        ✅ NUEVO: Detecta anomalías usando Z-score
        
        Retorna dict con:
        - z_score: valor del Z-score
        - severity: 'normal', 'warning', 'degraded', 'critical'
        - is_anomaly: bool
        - rolling_mean, rolling_std, rolling_p95: stats del historial
        """
        rolling_stats = self.calculate_rolling_stats(provider)
        
        # Si no hay suficiente historial, retornar normal
        ...
        # Calcular Z-score para peer latency
    ...
    def should_switch_provider(self) -> Tuple[str, str, Dict[str, Dict]]:
        """
        ✅ MEJORADO: Lógica de failover con umbrales dinámicos (Z-score)
        
        Flujo de decisión:
        1. Calcular scores y Z-scores para cada provider
        2. Si hay anomalía Z-score severa (≥3.0) → contar como degradación
        3. Si switch_margin se supera → contar como degradación
        4. Si degradation_counter >= SUSTAINED_DEGRADATION_CYCLES → FAILOVER
        """
        provider_scores = {}
        z_score_data = {}
        ...
        for provider in PROVIDERS:
            ...
            # ✅ NUEVO: Expandir historial a ROLLING_HISTORY_SIZE
            ...
            # ✅ NUEVO: Detectar anomalía Z-score ANTES de calcular score promedio
            anomaly_info = self.detect_anomaly_z_score(provider, metrics)
            z_score_data[provider] = anomaly_info
            
            # Calcular score promedio (últimos 3 ciclos para suavizar)
            ...
            provider_scores[provider] = {
                ...
                'z_score_info': anomaly_info
            }

        # Log de scores y Z-scores
        logging.info("📈 Scores promedio y Z-scores:")
        for provider in PROVIDERS:
            ...
            logging.info(
                ...
                f"Z={z_info['z_score']:+.2f}{severity_emoji} "
                f"(μ={z_info['rolling_mean']:.1f}, σ={z_info['rolling_std']:.1f}, n={z_info['history_count']})"
            )
        ...
        # ✅ NUEVO: Verificar si hay anomalía Z-score severa en el provider actual
        current_z_info = z_score_data[self.current_primary_provider]
        has_severe_anomaly = current_z_info['severity'] in ['degraded', 'critical']
        
        if best_provider != self.current_primary_provider:
            # ✅ LÓGICA MEJORADA: Verificar switch_margin O anomalía Z-score
            if score_diff > LATENCY_THRESHOLDS['switch_margin'] or has_severe_anomaly:
                
                # Determinar razón de degradación
                if has_severe_anomaly:
                    reason_type = f"anomalía Z-score ({current_z_info['severity']}, Z={current_z_info['z_score']:+.2f})"
                else:
                ...
                # ✅ FAILOVER cuando llega a N ciclos sostenidos
                ...
                # Return cuando aún no llega al umbral
                ...
                # Diferencia NO significativa → reiniciar contador
                self.degradation_counter = 0
                self.better_provider_candidate = None
                logging.info(f"✅ Diferencia insuficiente ({score_diff:.2f} < {LATENCY_THRESHOLDS['switch_margin']}) y sin anomalía Z-score")
        ...
        try:
            ...
            for provider_name, metrics in all_providers_metrics.items():
                metric = {
                    ...
                    # ✅ NUEVO: Campos de Z-score para análisis
    ...
    def _determine_quality_status(self, metrics: Dict) -> str:
        # ✅ MEJORADO: Considerar Z-score severity
        z_severity = metrics.get("z_score_severity", "normal")
        
        if metrics["has_latency_critical"] or metrics["peer_loss_pct"] >= 20:
            return "critical"
        elif z_severity in ['degraded', 'critical']:
            return "critical"
        elif metrics["has_latency_warning"] or metrics["peer_loss_pct"] > 0:
            return "warning"
        elif z_severity == 'warning':
            return "warning"
        return "excellent"
    ...
    def run_cycle(self):
        try:
            ...
            for provider in PROVIDERS:
                ...
                z_info = provider_scores[provider]['z_score_info']
                ...
                all_metrics[provider] = {
                    ...
                    # ✅ NUEVO: Métricas de Z-score
                    "z_score_peer": z_info['z_score'],
            ...
            if provider_will_change:
                ...
                # ✅ NUEVO: Limpiar historial del provider anterior
                # para evitar contaminación por Z-score
                self.metrics_history[self.last_provider] = []
                logging.info(f"🧹 Historial de {self.last_provider} limpiado (Z-score reset)")
            else:
...
def main():
    ...
    logging.info("🚀 BGP Failover Engine - Versión con Umbrales Dinámicos (Z-Score)")
